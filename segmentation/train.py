...
#    dataset_t = "Drishti-GS/"
    ...
    weights_path = "weights/" + dataset + "/DA_patch/_{epoch:04d}.hdf5"
    load_from = "./weights/weights1.h5"
    data_path = '/home/gpu3/shubham/refuge/Tsegmentation/'
...
#    _MODEL = os.path.basename(__file__).split('.')[0]
    ...
    logs_path = "./log_tf/" + dataset + "/DA_patch/"
    ...
    trainGenerator_Gene = Generator_Gene(batch_size, data_path + 'train', DiscROI_size,
                                         CDRSeg_size = CDRSeg_size, pt=False, phase='train')
#    if dataset_t in ['refuge']: # using val data to val
    valGenerator_Gene = Generator_Gene(batch_size, data_path + 'val', DiscROI_size,
                                           CDRSeg_size=CDRSeg_size, pt=False, phase='val')
#    else: # for other dataset using test data to val
#        valGenerator_Gene = Generator_Gene(batch_size, '/home/gpu3/shubham/refuge/Tsegmentation/val', DiscROI_size,
#                                           CDRSeg_size = CDRSeg_size, pt=False, phase='val')
#    if dataset_t in ['refuge']: # using val data to train without ground truth
    trainAdversarial_Gene = Adversarial_Gene(batch_size, data_path + 'val/', DiscROI_size,
...
#                                             CDRSeg_size=CDRSeg_size, phase='train', noise_label=False)
    trainDS_Gene = GD_Gene(batch_size, data_path + 'train', True,
                           CDRSeg_size=CDRSeg_size, phase='train', noise_label=False)
